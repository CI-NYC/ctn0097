...
arm_c <- read_csv("data/ctn0097dat_wide_armc.csv") |>
  as.data.frame() |>
  mutate(arm = 1)

arm_d <- read_csv("data/ctn0097dat_wide.csv") |>
           as.data.frame() |>
          mutate(arm = 0)

dat <- arm_c |> merge(arm_d, all = TRUE)
...
L <- list(c(W, "arm"), c("Y.0"), c("Y.1"), c("Y.2"), c("Y.3"), c("Y.4"))
...
saveRDS(results, "data/results_10_comb.rds")
...
saveRDS(results_origin, "data/results_origin_comb.rds")
...
saveRDS(results_5, "data/results_5_comb.rds")